...
public:
    ...
    void SetVolumes(int psgDb, int fmDb, int rhythmDb, int adpcmDb)
    {
        device.SetVolumePSG(psgDb);
        device.SetVolumeFM(fmDb);
    }
private:
	FM::OPN device;
	bool enable;
...
#ifdef EMSCRIPTEN
extern const char *getEmscriptenRhythmPath();
#endif
...
public:
	...
	void Init(Uint32 clock, Uint32 rate)
	{
		...
		device.Init(clock, rate, true, getEmscriptenRhythmPath());
#else		
		CHAR rhythmPath[MAX_PATH];
		LPSTR fnoff;
		DWORD ret;
		ret = GetFullPathName(GetDLLArgv0(), MAX_PATH, rhythmPath, &fnoff);
    ...
    void SetVolumes(int psgDb, int fmDb, int rhythmDb, int adpcmDb)
    {
        device.SetVolumePSG(psgDb);
        device.SetVolumeFM(fmDb);
        device.SetVolumeRhythmTotal(rhythmDb);
        device.SetVolumeADPCM(adpcmDb);
    }
private:
	FM::OPNA device;
	bool enable;
