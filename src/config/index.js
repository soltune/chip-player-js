...
// Fork: REACT_APP_* env vars (.env / .env.local) override the defaults above.
// Trailing slashes are stripped so both URL styles work with pathJoin.
const stripTrailingSlash = (url) => url && url.replace(/\/+$/, '');
API_BASE = stripTrailingSlash(process.env.REACT_APP_API_BASE) || API_BASE;
CATALOG_PREFIX = stripTrailingSlash(process.env.REACT_APP_CATALOG_PREFIX) || CATALOG_PREFIX;
SOUNDFONT_URL_PATH = stripTrailingSlash(process.env.REACT_APP_SOUNDFONT_URL_PATH) || SOUNDFONT_URL_PATH;
