...
const PMD_CHANNEL_COUNT = 2; // PMD always outputs interleaved stereo (L/R/L/R...)
const CHANNELS = [
...
export default class PMDPlayer extends Player {
  ...
  resampleToFloat(channels, channelId, inputPtr, len, resampleOutput, resampleLen) {
    ...
    for (let i = 0; i < resampleLen; i++) {
      ...
      // Use PMD_CHANNEL_COUNT as the input stride (PMD always outputs interleaved stereo),
      ...
      const p0 = this.readFloatSample(inputPtr, (i0 * PMD_CHANNEL_COUNT) + channelId);
      const p1 = this.readFloatSample(inputPtr, (i1 * PMD_CHANNEL_COUNT) + channelId);
      const p2 = this.readFloatSample(inputPtr, (i2 * PMD_CHANNEL_COUNT) + channelId);
      const p3 = this.readFloatSample(inputPtr, (i3 * PMD_CHANNEL_COUNT) + channelId);
