...
export default class XMPPlayer extends Player {
  ...
  _maybeInjectTempo(measuredBPM) {
    ...
    const minBPM = 20;
    const maxBPM = 255;
    const lastTargetBPM = Math.floor(Math.max(Math.min(this.lastBPM * this.tempoScale, maxBPM), minBPM));

    if (lastTargetBPM === measuredBPM) return;
    ...
    if (this.lastTempoScale === this.tempoScale) {  // tempo event received
      ...
      targetBPM = Math.floor(Math.max(Math.min(this.lastBPM * this.tempoScale, maxBPM), minBPM));
