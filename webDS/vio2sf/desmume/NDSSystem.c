...
void NDS_Reset( NDS_state *state)
{
    ...
    */
   {
	...
	{		
#ifdef EMSCRIPTEN
		u8 *mem= state->MMU->CART_ROM;
		int offset= i<<2;
		// supposedly the buffer here actually contains little endian:
		u32 val = ((u32)mem[offset+0]) | (((u32)mem[offset+1]) << 8) | (((u32)mem[offset+2]) << 16) | (((u32)mem[offset+3]) << 24);	// avoid unaligned access
		
		MMU_write32 (state, 0, 0x027FFE00+ (i<<2), val);	// avoid alignment issues (all little endian)
#else
