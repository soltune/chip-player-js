...
//		do_filter= do_suppressendsilence= false;
	...
	int32_t getCurrentPlayPosition() { return (double)data_written / (double)getSamplesRate() * 1000 + pos_delta; }
...
static input_twosf *g_input_2sf = NULL;
...
void ds_setup (void) {
    if (!g_input_2sf)  g_input_2sf = new input_twosf();
	if (!g_file) {
