	...
	case 0x00:	// Start Chips
		...
		if (VGMHead.lngHzC140)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = vgm_device_start_c140(CurChip, ChipClk, VGMHead.bytC140Type);
				CAA->StreamUpdate = &vgm_c140_update;
...
#ifdef EMSCRIPTEN
UINT8 GetChipType(UINT8 index)
{
    UINT8 num = 0;
	for (CA_LIST* CurLst = ChipListAll; CurLst != NULL; )
	{
	    if (num++ == index) {
	        return CurLst->CAud->ChipType;
	    }
		CurLst = CurLst->next;
	}
	return 0xFF;
...
static CA_LIST* GetCaList(UINT8 ChipType, UINT8 ChipID)
{
	for (CA_LIST* CurLst = ChipListAll; CurLst != NULL; )
	{
	    if (CurLst->CAud->ChipType == ChipType && CurLst->CAud->ChipID == ChipID) {
	        return CurLst;
	    }
		CurLst = CurLst->next;
	}
...
void SetMuteMask(UINT8 ChipType, UINT8 ChipID, UINT32 Mask1, UINT32 Mask2)
{
	if (ChipType == 0xFF)	// chip unused
		return;
	if (ChipType == 0x00 && ! UseFM)
		sn764xx_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x01 && ! UseFM)
		ym2413_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x02)
		ym2612_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x03)
		ym2151_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x04)
		segapcm_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x05)
		rf5c68_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x06)
		ym2203_set_mute_mask(ChipID, Mask1, Mask2);
	else if (ChipType == 0x07)
		ym2608_set_mute_mask(ChipID, Mask1, Mask2);
	else if (ChipType == 0x08)
		ym2610_set_mute_mask(ChipID, Mask1, Mask2);
	else if (ChipType == 0x09 && ! UseFM)
		ym3812_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x0A && ! UseFM)
		ym3526_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x0B && ! UseFM)
		y8950_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x0C && ! UseFM)
		ymf262_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x0D)
		ymf278b_set_mute_mask(ChipID, Mask1, Mask2);
	else if (ChipType == 0x0E)
		ymf271_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x0F)
		ymz280b_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x10)
		rf5c164_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x11)
	    GetCaList(ChipType, ChipID)->COpts->Disabled = (Mask1 & 0x01) == 0x01;
	else if (ChipType == 0x12 && ! UseFM)
		ayxx_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x13)
		gameboy_sound_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x14)
		nes_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x15)
		multipcm_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x16)
		GetCaList(ChipType, ChipID)->COpts->Disabled = (Mask1 & 0x01) == 0x01;
	else if (ChipType == 0x17)
		GetCaList(ChipType, ChipID)->COpts->Disabled = (Mask1 & 0x01) == 0x01;
	else if (ChipType == 0x18)
		okim6295_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x19)
		k051649_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x1A)
		k054539_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x1B)
		c6280_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x1C)
		c140_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x1D)
		k053260_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x1E)
		pokey_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x1F)
		qsound_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x20)
		scsp_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x21)
		ws_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x22)
		vsu_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x23)
		saa1099_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x24)
		es5503_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x25)
		es5506_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x26)
		x1_010_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x27)
		c352_set_mute_mask(ChipID, Mask1);
	else if (ChipType == 0x28)
		iremga20_set_mute_mask(ChipID, Mask1);
}

void SetSurround(bool EnableSurround)
{
    SurroundSound = EnableSurround;
}
#endif