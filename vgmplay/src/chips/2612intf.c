...
#ifdef ENABLE_ALL_CORES
	case EC_GENS:
		YM2612_Update((ym2612_ *) info->chip, DUMMYBUF, 0);
		YM2612_DacAndTimers_Update((ym2612_ *) info->chip, DUMMYBUF, 0);
		break;
	case EC_NUKED:
		break;
#endif
...
void ym2612_stream_update(UINT8 ChipID, stream_sample_t **outputs, int samples)
{
	...
	case EC_GENS:
		...
		YM2612_Update((ym2612_ *) info->chip, GensBuf, samples);
		YM2612_DacAndTimers_Update((ym2612_ *) info->chip, GensBuf, samples);
		...
		break;
	case EC_NUKED:
		OPN2_GenerateStream((ym3438_t *) info->chip, outputs, samples);
		break;
#endif
	}
}
...
#ifdef ENABLE_ALL_CORES
	case EC_GENS:
		YM2612_Reset((ym2612_ *) info->chip);
		break;
	case EC_NUKED:
		OPN2_Reset((ym3438_t *) info->chip, 0, 0);
		break;
#endif
	}
}
...
#ifdef ENABLE_ALL_CORES
	case EC_GENS:
		return YM2612_Read((ym2612_ *) info->chip);
	case EC_NUKED:
		return OPN2_Read((ym3438_t *) info->chip, offset);
#endif
	default:
		return 0x00;
	}
...
#ifdef ENABLE_ALL_CORES
	case EC_GENS:
		YM2612_Write((ym2612_ *) info->chip, (unsigned char)(offset & 0x03), data);
		break;
	case EC_NUKED:
		OPN2_WriteBuffered((ym3438_t *) info->chip, offset, data);
		break;
#endif
	}
}
...
#ifdef ENABLE_ALL_CORES
	case EC_GENS:
		YM2612_SetMute((ym2612_ *) info->chip, (int)MuteMask);
		break;
	case EC_NUKED:
		OPN2_SetMute((ym3438_t *) info->chip, MuteMask);
		break;
#endif
	}
