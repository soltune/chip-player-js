...
/*static TIMER_CALLBACK( timer_callback_0 )
{
	...
	vgm_ym3812_timer_over(info->chip,0);
}

static TIMER_CALLBACK( timer_callback_1 )
{
	ym3812_state *info = (ym3812_state *)ptr;
	vgm_ym3812_timer_over(info->chip,1);
}*/
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		vgm_ym3812_update_one(info->chip, outputs, samples);
		break;
#endif
	case EC_DBOPL:
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		vgm_ym3812_update_one(info->chip, DUMMYBUF, 0);
		break;
#endif
	case EC_DBOPL:
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		info->chip = vgm_ym3812_init(clock & 0x7FFFFFFF,rate);
		...
		/* YM3812 setup */
		vgm_ym3812_set_timer_handler (info->chip, TimerHandler, info);
		vgm_ym3812_set_irq_handler   (info->chip, IRQHandler, info);
		vgm_ym3812_set_update_handler(info->chip, _stream_update, info);
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		vgm_ym3812_reset_chip(info->chip);
		break;
#endif
	case EC_DBOPL:
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		return vgm_ym3812_read(info->chip, offset & 1);
#endif
	case EC_DBOPL:
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		vgm_ym3812_write(info->chip, offset & 1, data);
		break;
#endif
	case EC_DBOPL:
