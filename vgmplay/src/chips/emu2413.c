...
void
_OPLL_dump2patch (const e_uint8 * dump, OPLL_PATCH * patch)
{
...
void
_OPLL_getDefaultPatch (e_int32 type, e_int32 num, OPLL_PATCH * patch)
{
  _OPLL_dump2patch (default_inst[type] + num * 8, patch);
}
...
makeDefaultPatch ()
{
  ...
  for (i = 0; i < OPLL_TONE_NUM; i++)
    for (j = 0; j < 19; j++)
      _OPLL_getDefaultPatch (i, j, &default_patch[i][j * 2]);

}

void
_OPLL_setPatch (OPLL * opll, const e_uint8 * dump)
{
  OPLL_PATCH patch[2];
  int i;
  ...
  for (i = 0; i < 19; i++)
  {
    _OPLL_dump2patch (dump + i * 8, patch);
...
void
_OPLL_patch2dump (const OPLL_PATCH * patch, e_uint8 * dump)
{
...
void
_OPLL_copyPatch (OPLL * opll, e_int32 num, OPLL_PATCH * patch)
{
...
OPLL *
_OPLL_new (e_uint32 clk, e_uint32 rate)
{
  OPLL *opll;
  e_int32 i;
...
void
_OPLL_reset_patch (OPLL * opll, e_int32 type)
{
  e_int32 i;

  for (i = 0; i < 19 * 2; i++)
    _OPLL_copyPatch (opll, i, &default_patch[type % OPLL_TONE_NUM][i]);
}

/* Reset whole of OPLL except patch datas. */
void
_OPLL_reset (OPLL * opll)
{
  e_int32 i;
...
void
_OPLL_set_rate (OPLL * opll, e_uint32 r)
{
  if (opll->quality)
    rate = 49716;
  else
...
void
_OPLL_set_quality (OPLL * opll, e_uint32 q)
{
  opll->quality = q;
  _OPLL_set_rate (opll, rate);
}
...
void _OPLL_SetMuteMask(OPLL* opll, e_uint32 MuteMask)
{
...
void _OPLL_SetChipMode(OPLL* opll, e_uint8 Mode)
{
...
void
_OPLL_writeReg (OPLL * opll, e_uint32 reg, e_uint32 data)
{
  e_int32 i, v, ch;
...
void
_OPLL_writeIO (OPLL * opll, e_uint32 adr, e_uint32 val)
{
  if (adr & 1)
    _OPLL_writeReg (opll, opll->adr, val);
  else
    opll->adr = val;
}
...
/* STEREO MODE (OPT) */
void
_OPLL_set_pan (OPLL * opll, e_uint32 ch, e_int32 pan)
{
	e_uint32 fnl_ch;
...
void
_OPLL_calc_stereo (OPLL * opll, e_int32 **out, e_int32 samples)
{
  e_int32 *bufMO = out[0];
  e_int32 *bufRO = out[1];
  e_int32 buffers[2];
