...
struct SOUND
{
	...
	UINT8  length;
	UINT8  length_mask;
	gb_bool   length_counting;
	gb_bool   length_enabled;
	/* Mode 1, 2, 3 */
	UINT32 cycles_left;
	INT8   duty;
	/* Mode 1, 2, 4 */
	gb_bool   envelope_enabled;
	INT8   envelope_value;
	...
	gb_bool   sweep_enabled;
	gb_bool   sweep_neg_mode_used;
	UINT8  sweep_shift;
...
INLINE gb_bool gb_dac_enabled(struct SOUND *snd);
...
void gb_sound_w_internal(gb_sound_t *gb, UINT8 offset, UINT8 data)
{
	...
	case NR14: /* Frequency hi / Initialize (R/W) */
		gb->snd_1.reg[4] = data;
		{
			gb_bool length_was_enabled = gb->snd_1.length_enabled;
	...
	case NR24: /* Frequency hi / Initialize (R/W) */
		gb->snd_2.reg[4] = data;
		{
			gb_bool length_was_enabled = gb->snd_2.length_enabled;
	...
	case NR34: /* Frequency hi / Initialize (W) */
		gb->snd_3.reg[4] = data;
		{
			gb_bool length_was_enabled = gb->snd_3.length_enabled;
	...
	case NR44: /* Counter/Consecutive / Initialize (R/W)  */
		gb->snd_4.reg[4] = data;
		{
			gb_bool length_was_enabled = gb->snd_4.length_enabled;
