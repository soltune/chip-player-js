...
/*static TIMER_CALLBACK( timer_callback_0 )
{
	...
	vgm_y8950_timer_over(info->chip,0);
}
static TIMER_CALLBACK( timer_callback_1 )
{
	y8950_state *info = (y8950_state *)ptr;
	vgm_y8950_timer_over(info->chip,1);
}*/
...
void y8950_stream_update(UINT8 ChipID, stream_sample_t **outputs, int samples)
{
	...
	vgm_y8950_update_one(info->chip, outputs, samples);
}
...
static void _stream_update(void *param/*, int interval*/)
{
	...
	vgm_y8950_update_one(info->chip, DUMMYBUF, 0);
}
...
int device_start_y8950(UINT8 ChipID, int clock)
{
	...
	info->chip = vgm_y8950_init(clock,rate);
	...
	vgm_y8950_set_delta_t_memory(info->chip, NULL, 0x00);

	//info->stream = stream_create(device,0,1,rate,info,y8950_stream_update);

	/* port and keyboard handler */
	vgm_y8950_set_port_handler(info->chip, Y8950PortHandler_w, Y8950PortHandler_r, info);
	vgm_y8950_set_keyboard_handler(info->chip, Y8950KeyboardHandler_w, Y8950KeyboardHandler_r, info);

	/* Y8950 setup */
	vgm_y8950_set_timer_handler (info->chip, TimerHandler, info);
	vgm_y8950_set_irq_handler   (info->chip, IRQHandler, info);
	vgm_y8950_set_update_handler(info->chip, _stream_update, info);
...
void device_reset_y8950(UINT8 ChipID)
{
	//y8950_state *info = get_safe_token(device);
	y8950_state *info = &Y8950Data[ChipID];
	vgm_y8950_reset_chip(info->chip);
}
...
UINT8 y8950_r(UINT8 ChipID, offs_t offset)
{
	//y8950_state *info = get_safe_token(device);
	y8950_state *info = &Y8950Data[ChipID];
	return vgm_y8950_read(info->chip, offset & 1);
}
...
void y8950_w(UINT8 ChipID, offs_t offset, UINT8 data)
{
	//y8950_state *info = get_safe_token(device);
	y8950_state *info = &Y8950Data[ChipID];
	vgm_y8950_write(info->chip, offset & 1, data);
}
							  ...
							  offs_t DataLength, const UINT8* ROMData)
{
	...
	vgm_y8950_write_pcmrom(info->chip, ROMSize, DataStart, DataLength, ROMData);
