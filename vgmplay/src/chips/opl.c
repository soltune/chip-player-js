...
static void adlib_write(void *chip, Bitu idx, Bit8u val)
{
	...
	case ARC_TVS_KSR_MUL+0x10:
		...
		if ((num<6) && (base<22)) {
			...
			change_keepsustain((OPL_DATA *) chip, regbase,op_ptr);
			change_vibrato((OPL_DATA *) chip, regbase,op_ptr);
			...
			if ((OPL->adlibreg[0x105]&1) && (OPL->op[modop].is_4op_attached))
			{
				...
				change_frequency((OPL_DATA *) chip, chanbase-3,regbase,op_ptr);
			}
			else
			{
				change_frequency((OPL_DATA *) chip, chanbase,regbase,op_ptr);
			}
#else
			change_frequency((OPL_DATA *) chip, chanbase,base,op_ptr);
#endif
		}
		}
		break;
	...
	case ARC_ATTR_DECR+0x10:
		...
		if ((num<6) && (base<22))
		{
			...
			change_attackrate((OPL_DATA *) chip, regbase,op_ptr);
			change_decayrate((OPL_DATA *) chip, regbase,op_ptr);
		}
		}
		break;
	case ARC_SUSL_RELR:
	case ARC_SUSL_RELR+0x10:
		...
		if ((num<6) && (base<22))
		{
			...
			op_type* op_ptr = &OPL->op[regbase2op[second_set?(base+22):base]];
			change_releaserate((OPL_DATA *) chip, regbase,op_ptr);
			change_sustainlevel((OPL_DATA *) chip, regbase,op_ptr);
		}
		}
		break;
	case ARC_FREQ_NUM:
		...
		if ((OPL->adlibreg[0x105]&1) && OPL->op[opbase].is_4op_attached) break;
			...
			change_frequency((OPL_DATA *) chip, chanbase,modbase,&OPL->op[opbase]);
			change_frequency((OPL_DATA *) chip, chanbase,modbase+3,&OPL->op[opbase+9]);
#if defined(OPLTYPE_IS_OPL3)
			...
			{
				change_frequency((OPL_DATA *) chip, chanbase,modbase+8,&OPL->op[opbase+3]);
				change_frequency((OPL_DATA *) chip, chanbase,modbase+3+8,&OPL->op[opbase+3+9]);
			}
#endif
		}
		}
	...
	case ARC_KON_BNUM:
		...
		if (idx == ARC_PERC_MODE)
		{
			...
			if ((val&0x30) == 0x30)
			{		// BassDrum active
				enable_operator((OPL_DATA *) chip, 16,&OPL->op[6],OP_ACT_PERC);
				change_frequency((OPL_DATA *) chip, 6,16,&OPL->op[6]);
				enable_operator((OPL_DATA *) chip, 16+3,&OPL->op[6+9],OP_ACT_PERC);
				change_frequency((OPL_DATA *) chip, 6,16+3,&OPL->op[6+9]);
			}
			else
			{
			...
			if ((val&0x28) == 0x28)
			{		// Snare active
				enable_operator((OPL_DATA *) chip, 17+3,&OPL->op[16],OP_ACT_PERC);
				change_frequency((OPL_DATA *) chip, 7,17+3,&OPL->op[16]);
			}
			else
			{
			...
			if ((val&0x24) == 0x24)
			{		// TomTom active
				enable_operator((OPL_DATA *) chip, 18,&OPL->op[8],OP_ACT_PERC);
				change_frequency((OPL_DATA *) chip, 8,18,&OPL->op[8]);
			}
			else
			{
			...
			if ((val&0x22) == 0x22)
			{		// Cymbal active
				enable_operator((OPL_DATA *) chip, 18+3,&OPL->op[8+9],OP_ACT_PERC);
				change_frequency((OPL_DATA *) chip, 8,18+3,&OPL->op[8+9]);
			}
			else
			{
			...
			if ((val&0x21) == 0x21)
			{		// Hihat active
				enable_operator((OPL_DATA *) chip, 17,&OPL->op[7],OP_ACT_PERC);
				change_frequency((OPL_DATA *) chip, 7,17,&OPL->op[7]);
			}
			else
			{
		...
		if (base<9)
		{
			...
			if (val&32)
			{
				...
				enable_operator((OPL_DATA *) chip, modbase,&OPL->op[opbase],OP_ACT_NORMAL);		// modulator (if 2op)
				enable_operator((OPL_DATA *) chip, modbase+3,&OPL->op[opbase+9],OP_ACT_NORMAL);	// carrier (if 2op)
#if defined(OPLTYPE_IS_OPL3)
				...
				if ((OPL->adlibreg[0x105]&1) && OPL->op[opbase].is_4op)
				{
					...
					enable_operator((OPL_DATA *) chip, modbase+8,&OPL->op[opbase+3],OP_ACT_NORMAL);
					enable_operator((OPL_DATA *) chip, modbase+3+8,&OPL->op[opbase+3+9],OP_ACT_NORMAL);
				}
#endif
			}
			else
	...
	case ARC_FEEDBACK:
		...
		if (base<9)
		{
			...
			change_feedback((OPL_DATA *) chip, chanbase,&OPL->op[opbase]);
#if defined(OPLTYPE_IS_OPL3)
			// OPL3 panning
	...
	case ARC_WAVE_SEL+0x10:
		...
		if ((num<6) && (base<22))
		{
			...
			change_waveform((OPL_DATA *) chip, wselbase,op_ptr);
#else
			if (OPL->adlibreg[0x01]&0x20)
			{
				...
				change_waveform((OPL_DATA *) chip, base,op_ptr);
			}
#endif
		}
		}
