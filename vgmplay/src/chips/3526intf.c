...
/*static TIMER_CALLBACK( timer_callback_0 )
{
	...
	vgm_ym3526_timer_over(info->chip,0);
}
static TIMER_CALLBACK( timer_callback_1 )
{
	ym3526_state *info = (ym3526_state *)ptr;
	vgm_ym3526_timer_over(info->chip,1);
}*/
...
void ym3526_stream_update(UINT8 ChipID, stream_sample_t **outputs, int samples)
{
	...
	vgm_ym3526_update_one(info->chip, outputs, samples);
}
...
static void _stream_update(void *param/*, int interval*/)
{
	...
	vgm_ym3526_update_one(info->chip, DUMMYBUF, 0);
}
...
int device_start_ym3526(UINT8 ChipID, int clock)
{
	...
	info->chip = vgm_ym3526_init(clock,rate);
	...
	/* YM3526 setup */
	vgm_ym3526_set_timer_handler (info->chip, TimerHandler, info);
	vgm_ym3526_set_irq_handler   (info->chip, IRQHandler, info);
	vgm_ym3526_set_update_handler(info->chip, _stream_update, info);
...
void device_reset_ym3526(UINT8 ChipID)
{
	//ym3526_state *info = get_safe_token(device);
	ym3526_state *info = &YM3526Data[ChipID];
	vgm_ym3526_reset_chip(info->chip);
}
...
UINT8 ym3526_r(UINT8 ChipID, offs_t offset)
{
	//ym3526_state *info = get_safe_token(device);
	ym3526_state *info = &YM3526Data[ChipID];
	return vgm_ym3526_read(info->chip, offset & 1);
}
...
void ym3526_w(UINT8 ChipID, offs_t offset, UINT8 data)
{
	//ym3526_state *info = get_safe_token(device);
	ym3526_state *info = &YM3526Data[ChipID];
	vgm_ym3526_write(info->chip, offset & 1, data);
}
