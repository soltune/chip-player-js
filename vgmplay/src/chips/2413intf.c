...
#endif
	case EC_EMU2413:
		_OPLL_calc_stereo((OPLL *) info->chip, outputs, samples);
		break;
	}
}
...
#endif
	case EC_EMU2413:
		_OPLL_calc_stereo((OPLL *) info->chip, DUMMYBUF, 0);
		break;
	}
}
...
int device_start_ym2413(UINT8 ChipID, int clock)
{
	...
	case EC_MAME:
		...
		ym2413_set_update_handler((OPLL *) info->chip, _stream_update, info);
		break;
#endif
	case EC_EMU2413:
		info->chip = _OPLL_new(clock, rate);
		if (info->chip == NULL)
			return 0;
		
		_OPLL_SetChipMode((OPLL *) info->chip, info->Mode);
		if (info->Mode)
			_OPLL_setPatch((OPLL *) info->chip, vrc7_inst);
		break;
	}
...
void device_reset_ym2413(UINT8 ChipID)
{
	...
	case EC_MAME:
		...
		if (info->Mode)
			ym2413_override_patches((OPLL *) info->chip, vrc7_inst);
		break;
#endif
	case EC_EMU2413:
		_OPLL_reset((OPLL *) info->chip);
...
#endif
	case EC_EMU2413:
		_OPLL_writeIO((OPLL *) info->chip, offset & 1, data);
		break;
	}
}
...
#endif
	case EC_EMU2413:
		_OPLL_SetMuteMask((OPLL *) info->chip, MuteMask);
		break;
	}
...
void ym2413_set_panning(UINT8 ChipID, INT16* PanVals)
{
	...
	case EC_EMU2413:
		for (CurChn = 0x00; CurChn < 0x0E; CurChn ++)
			_OPLL_set_pan((OPLL *) info->chip, CurChn, PanVals[CurChn]);
		break;
	}
