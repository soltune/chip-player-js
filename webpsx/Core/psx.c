...
sint32 EMU_CALL psx_upload_psxexe(void *state, void *program, uint32 size) {
  ...
  // Try to determine the region, or leave it at the default if it's not found
       if(string_exists((const char*) program, 0x800, "North America")) { psx_set_refresh(state, 60); }
  else if(string_exists((const char*) program, 0x800, "Japan"        )) { psx_set_refresh(state, 60); }
  else if(string_exists((const char*) program, 0x800, "Europe"       )) { psx_set_refresh(state, 50); }
