...
static void init_mod_fields(
  ...
  strncpy((char *__restrict) mod->name, name, 10);
  mod->code  = code;
  mod->start = start;
  mod->size  = size;
}
...
static int find_romdir_entry(
  ...
  for(; romdir_start < bios_size; romdir_start += 0x10) {
    ...
    if(!strcmp((const char*) n, (const char*) name)) {
...
static struct MODULE *master_find_module(struct MKHEBIOS *state, const char *name) {
  ...
  for(n = 0; n < state->master_module_pin_n; n++) {
    if(!strcasecmp((const char*) state->master_module_pin_list[n].name, name)) return state->master_module_pin_list + n;
  }
  for(n = 0; n < state->master_module_free_n; n++) {
    if(!strcasecmp((const char*) state->master_module_free_list[n].name, name)) return state->master_module_free_list + n;
  }
  return NULL;
}
...
static void master_module_free_add(struct MKHEBIOS *state, struct MODULE *mod) {
  state->master_module_free_list = (struct MODULE*) realloc(
...
static void master_module_pin_add(struct MKHEBIOS *state, struct MODULE *mod) {
  state->master_module_pin_list = (struct MODULE*) realloc(
...
static void rom_usage_clear(struct MKHEBIOS * state) {
  ...
  state->rom_usage_map = (struct MODULE**) realloc( state->rom_usage_map, sizeof(struct MODULE *) * state->usage_entries );
...
static int line_from_module(struct MKHEBIOS * state, struct MODULE *m) { return line_from_arbitrary(state, (const char*) m->name, m->code, m->size); }
...
try_file:
  ...
  strcpy((char *__restrict) mod.name, n);
...
static int mkhebios_script_rebuild(struct MKHEBIOS *state, const char *args) {
  ...
  for(
    ...
    state->romdir = (uint8_t*) malloc(state->romdir_size);
    if(!state->romdir) { log_error; return 1; }
...
void expandOverlays() {
	...
	for (int i= 0; i<sizeof(modules_list); i++) {
		...
		uint8 * dataDest= (uint8_t*)  malloc(destSize);
		
		long newSize= modules_list[i].size;
		if (Z_OK != uncompress(dataDest, (uLongf*) &destSize, modules_list[i].data, modules_list[i].size)) {
