...
void EMU_CALL spu_dma(void *state, uint32 core, void *mem, uint32 mem_ofs, uint32 mem_mask, uint32 bytes, int iswrite) {
  ...
  if(iswrite) {
    while(words--) {
      ...
      set_transfer((struct SPU_STATE*) state, core, d      );
      set_transfer((struct SPU_STATE*) state, core, d >> 16);
      mem_ofs += 4;
    }
  } else {
    while(words--) {
      uint32 d;
      mem_ofs &= mem_mask;
      d  = ((uint32)(get_transfer((struct SPU_STATE*) state, core)))      ;
      d |= ((uint32)(get_transfer((struct SPU_STATE*) state, core))) << 16;
...
//  mainout = 0;
  if(SPUSTATE->version == 1) {
    spucore_render(CORESTATE(0), (uint16_t*) SPURAM, buf, NULL, samples, mainout, effectout);
  } else {
    spucore_render(CORESTATE(0), (uint16_t*) SPURAM, buf, NULL, samples, mainout, effectout);
    spucore_render(CORESTATE(1), (uint16_t*) SPURAM, buf, buf , samples, mainout, effectout);
...
//  mainout = 0;
  if(SPUSTATE->version == 1) {
    spucore_render(CORESTATE(0), (uint16_t*) SPURAM, buf, ext, samples, mainout, effectout);
  } else {
    spucore_render(CORESTATE(0), (uint16_t*) SPURAM, buf, ext, samples, mainout, effectout);
    spucore_render(CORESTATE(1), (uint16_t*) SPURAM, buf, buf, samples, mainout, effectout);
//    spucore_render(CORESTATE(1), SPURAM, buf, NULL, samples);
...
uint32 EMU_CALL spu_cycles_until_interrupt(void *state, uint32 samples) {
  if(SPUSTATE->version == 1) {
    return spucore_cycles_until_interrupt(CORESTATE(0), (uint16_t*) SPURAM, samples);
  } else {
    uint32 cycles1 = spucore_cycles_until_interrupt(CORESTATE(0), (uint16_t*) SPURAM, samples);
    uint32 cycles2 = spucore_cycles_until_interrupt(CORESTATE(1), (uint16_t*) SPURAM, samples);
