...
int isCompressed(const char *filename) {
	char* point;
	if((point = (char*) strrchr(filename,'.')) != NULL ) {
...
//		if ((he_bios_path == 0) || !strlen(he_bios_path)) {
			...
			if (Z_OK !=  uncompress((Bytef*) he_bios, (uLongf*) &hebios_size, zhebios, zhebios_size)) {
			...
			bios_set_image( (uint8_t*) he_bios, hebios_size );
...
//		} else {
			...
			FILE * f = (FILE*) psf_file_fopen( he_bios_path );
			if ( !f ) {
			...
			bios_set_image( (uint8_t*) he_bios, bios_size );
//		}
#endif
	}
	return 0;
...
int he_init (DB_fileinfo_t *_info, const char * uri) {
 ...
 //   int psf_version = psf_load( uri, &psf_file_system, 0, 0, 0, 0, 0, 0 );
    ...
    _info->fmt.psf_version = psf_version;
	_info->readpos = 0;
...
void he_set_reverb(DB_fileinfo_t *_info, uint8 enable) {
    he_info_t *info = (he_info_t *)_info;
    spu_enable_reverb(iop_get_spu_state(psx_get_iop_state(info->emu)), enable);
}
